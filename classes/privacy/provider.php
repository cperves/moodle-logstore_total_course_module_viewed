...
/**
 * logstore total course module viewed provider class
 *
...
namespace logstore_total_course_module_viewed\privacy;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        $collection->add_database_table('logstore_totalcoursemodview', [
                'userid' => 'privacy:metadata:log:userid',
                'courseid' => 'privacy:metadata:log:courseid',
                'totalcoursemoduleviewed' => 'privacy:metadata:log:totalcoursemoduleviewed',
                'totalcoursemoduleviewed' => 'privacy:metadata:log:totalcourseresourcesviewed',
                'totalcoursemoduleviewed' => 'privacy:metadata:log:totalcourseactivitiesviewed',
    ...
    public static function add_contexts_for_userid(contextlist $contextlist, $userid) {
        $sql = "
            SELECT ctx.id
              FROM {logstore_totalcoursemodview} l
    ...
    public static function add_userids_for_context(\core_privacy\local\request\userlist $userlist) {
        ...
        $sql = "
            SELECT userid
              FROM {logstore_totalcoursemodview}
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        foreach ($aprovedcontextlist as $approvedcontext) {
            ...
            $entries = $DB->get_records('logstore_totalcoursemodview', $params);
            if (!empty($entries)) {
                writer::with_context($approvedcontext)->export_data(
                        [get_string('pluginname', 'logstore_total_course_module_viewed')],
                        (object)['logstore_totalcoursemodview_records' => $entries]
                );
            }
        }
    }
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        global $DB;
        if ($context instanceof \context_course) {
            $DB->delete_records_select('logstore_totalcoursemodview', 'courseid=:courseid'
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            if ($context instanceof \context_course) {
                $DB->delete_records_select('logstore_totalcoursemodview',
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        foreach ($userlist as $user) {
            if ($context instanceof \context_course) {
                $DB->delete_records_select('logstore_coursemoduleviewed',
                    'courseid=:courseid and userid=:userid',
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        $context = $userlist->get_context();
        if ($context instanceof \context_course) {
            $sql = 'select userid from {logstore_totalcoursemodview} where courseid=:courseid';
            $params = [
                    'courseid' => $context->instanceid
            ];
            $userlist->add_from_sql('userid', $sql, $params);
    ...
    public static function get_contexts_for_userid(int $userid) : contextlist {
        ...
        $sql = 'select ctx.id from {logstore_totalcoursemodview} ucc
