...
namespace logstore_total_course_module_viewed\task;
...
class cleanup_task extends \core\task\scheduled_task {
    ...
    public function get_name() {
        return get_string('taskcleanup', 'logstore_total_course_module_viewed');
    }
    ...
    public function execute() {
        ...
        $loglifetime = (int)get_config('logstore_total_course_module_viewed', 'loglifetime');
        ...
        while ($min = $DB->get_field_select("logstore_totalcoursemodview",
                        "MIN(timemodified)", "timemodified < ?", $lifetimep)
        ) {
            ...
            $DB->delete_records_select("logstore_totalcoursemodview", "timemodified < ?", $params);
            if (time() > $start + 300) {
        ...
        mtrace(" Deleted old log records from  total_course_module_viewed log store.");
    }
}
